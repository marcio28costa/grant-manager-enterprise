...
public class VinculoPanel extends JPanel {
    ...
    private JComboBox<String> cbUsuario, cbIp, cbPermissao;
    private JButton btnVincular, btnExcluir, btnAtualizar; // Adicionado btnAtualizar
    private JTable tabelaVinculos;
    private DefaultTableModel model;
    ...
    public VinculoPanel() {
        ...
        // --- PAINEL DE CADASTRO (NORTE) ---
        // Alterado para (5, 2) para caber o novo botão
        ...
        btnAtualizar = new JButton("🔄 Atualizar Listas"); // Botão novo
        btnAtualizar.setToolTipText("Clique aqui se cadastrou novos usuários ou IPs agora");
        ...
        // Linha de ações
        pnlForm.add(btnAtualizar); // Adicionado na esquerda
        pnlForm.add(btnVincular);  // Adicionado na direita
    ...
    // Alterado para public para o MainFrame conseguir chamar ao trocar de aba
    public void atualizarTodosOsCombos() {
        carregarCombo("SELECT ID, USERNAME FROM usuarios ORDER BY USERNAME", cbUsuario, mapUsuarios, "USERNAME");
        carregarCombo("SELECT ID, ENDERECO_IP FROM enderecosip ORDER BY ENDERECO_IP", cbIp, mapIps, "ENDERECO_IP");
        carregarCombo("SELECT ID, CONCAT(PERMISSAO, ' (', EXTRA, ')') as NOME FROM permissoes ORDER BY PERMISSAO", cbPermissao, mapPermissoes, "NOME");
    }
    ...
    private void configurarEventos() {
        ...
        btnAtualizar.addActionListener(e -> {
            ...
            btnAtualizar.setText("✅ Listas Sincronizadas");
            Timer timer = new Timer(2000, evt -> btnAtualizar.setText("🔄 Atualizar Listas"));
        ...
        btnVincular.addActionListener(e -> {
            ...
            try (Connection conn = ConnectionFactory.getConnection();
                ...
                JOptionPane.showMessageDialog(this, "Este vínculo já existe no banco de dados.");
            }
        });
